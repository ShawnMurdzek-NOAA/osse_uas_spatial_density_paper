...
# Y-axis label
ylabel = {'MLCAPE': 'MLCAPE diff (J kg$^{-1}$)',
          'MLCIN': 'MLCIN diff (J kg$^{-1}$)',
          'SRH03': '0$-$3 km SRH diff (m$^{2}$ s$^{-2}$)'}
...
for i in range(4):
    ...
    # Axes limits
    if i == 1: ax.set_ylim(top=40)
    if i == 2: ax.set_ylim(top=5)
    if i == 3: ax.set_ylim(top=8)

plt.suptitle('RMSEs for gridpoints with MUCAPE > 50 J kg$^{-1}$', size=16)
